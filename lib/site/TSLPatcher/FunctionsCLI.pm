...
sub UpdateProgress {
    ...
    # print "$log_index / $log_count: "
    #   . ( ( $log_index / $log_count ) * 100 ) . "\n";
}
    ...
    if ( $InstallInfo{bInstall} == 1 ) {
        ...
        if ( ( -e "$install_dest_path\\$destination\\$saveasfile" ) == 0 ) {
            if ( ( -e "$install_path\\$sourcefile" ) == 0 ) {
                ...
                # TODO: $located is not defined here?
                my $located = undef;    # TODO
                if ( $located ne '' ) {
                    ...
                    ProcessMessage(
                        Format(
                            $Messages{LS_EXC_FHCRITFILEMISSING}, $saveasfile
                        ),
                        LOG_LEVEL_ERROR
                    );
                    return ( 0, '' );
    ...
    if ( substr( $key, 0, 9 ) eq '2DAMEMORY' ) {
            ...
            or $action == ACTION_MODIFY_ROW )
        {
            ...
            if ( $value eq 'ColumnLabel' ) {
                ...
                # $row is being set here but was not declared or used later?
                my $row = undef;
                if (    ( lc( substr( $value, 0, 1 ) ) eq 'i' )
                    and ( $tmp >= 0 ) )
                {
                    $row = $tmp;
                }
...
sub Delete2daRow {
    ...
    # TODO: $modify_row was not defined here?
    my $modify_row = undef;
    ProcessMessage(
        Format(
...
sub Copy2daRow {
    ...
    # TODO: $value was not defined here?
    my $value = undef;
    if ( $value ne '' ) {
        my @answer = CheckForNonExclusiveLabel( $twoda, $section, $exclusive );
                    ...
                    if ( $changes > 0 ) {
                        ...
                        if ( $PatchType eq 'fileGFFERF' ) {
                            ...
                            if ( ( "modules/$ERF_name" ~~ @ERFs ) == 0 ) {
                                ...
                                # Set whether to treat this as an ERF or a RIM
                                # TODO: $dest and $folder were not defined here?
                                my $dest   = undef;
                                my $folder = undef;
                                if   ( $dest =~ /\.rim$/ ) { $IsERF = 0; }
                                else                       { $IsERF = 1; }
...
sub DoCompileFiles {
    ...
    if ( ( -e $k_inc_compile_path ) == 1 ) {
        File::Copy::copy( $k_inc_compile_path,
            "$base/tslpatchdata/nsstemp/k_inc_utility.nss" );
    }
    ...
    # TODO @Orgs was not defined here?
    my @Orgs = ();
    foreach $piece (@lines1) {
                ...
                if ( -e $FileBase )    # "$base/tslpatchdata/$NCSFile")
                {
                    ...
                    if ( lc($dest) ne 'override' ) {
                        ...
                        if ( -e "$install_path/$ERF_name" ) {
                            ...
                            # Make a backup in the Backup folder
                            # TODO $folder was not defined here?
                            my $folder = undef;
                            if (
                                (
                                    MakeBackup(
...
sub DoCleanup {
    ...
    if ( ( scalar @ERFs ) > 0 ) {
        ...
        foreach $ERF_name (@ERF_delete) {
            ...
            next
              if ( !($ERF_name) )
              ;    # Empty ERF_name will delete all data in the install path
    ...
    while (<FH>) {
        if ( $_ =~ /^\{\\rtf/ ) {
            ...
            foreach ( 0 .. ( scalar @font - 1 ) ) {
                ...
                # print "$_ $f\n";
                # unshift @font; # This does nothing
