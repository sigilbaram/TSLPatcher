...
"Routine get_cell lacks a Row Number to index, as well as a Column Header to find the cell...\n"
        ...
        $_[0]->set_err(
            "Routine get_cell lacks a Column Header to find the cell...\n");
        return undef;
    }
...
"Routine change_cell lacks a Row Number to index and a Column Header to find the cell...\n"
        ...
        $_[0]->set_err(
            "Routine change_cell lacks a Column Header to find the cell...\n");
        return undef;
    }
...
sub get_row_number {
    ...
    return ( List::Util::first { $_ == $row_header },
        @{ $self->{rows_array} } );
}
...
sub read2da_asarray {
    ...
    my $file_to_read = shift;
    if ( ref $file_to_read eq 'SCALAR' ) {
        return read2da_asarray_scalar( $self, $file_to_read );
    }
    if ( ref $file_to_read eq 'GLOB' ) {
        return read2da_asarray_fh( $self, $file_to_read );
    }
...
sub write2da {
    ...
    my $data_start_pos = sysseek( FH, 0, 1 ) +
      ( 2 * $self->{rows} * ( scalar @{ $self->{columns} } ) ) + 2;
