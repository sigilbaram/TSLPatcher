...
sub as_scalar() {
    ...
    $struct->writeHeaderScalar( undef, $sh, undef,
...
sub writeField {
    ...
    {    # byte, char, word, short, dword, int  -- all nullpadded to 4 bytes
        ...
        for my $exolocsubstr (@exolocsubstrs) {
            ...
            $packed_substrs .=
              pack( 'V V', $exolocsubstr->{'StringID'}, $exolocsubstr_len )
              . $exolocsubstr->{'Value'};
        }
...
sub createField {
    ...
    if ( $testhash{'Type'} == 12 ) {    # special case for CExoLocString
        ...
        if ( defined($cexolocsub) ) {
            $cexoloc = Bioware::GFF::CExoLocString->new(
                'StringRef'  => $testhash{'StringRef'},
                'Substrings' => [$cexolocsub]
            );
        }
        else {
            $cexoloc = Bioware::GFF::CExoLocString->new(
        ...
        $new_field = Bioware::GFF::Field->new(
            'Type'  => 12,
            'Label' => $testhash{'Label'},
            'Value' => $cexoloc
        );
    }
    else {
...
sub writeHeader {
    ...
    sysread $write_info{'fh_struct'}, my $struct_data,
    ...
    sysread $write_info{'fh_label'}, my $label_data,
      ( -s $write_info{'fh_label'} );
    $total_written += syswrite $fh, $label_data;

    sysseek $write_info{'fh_fielddata'}, 0, 0;
    sysread $write_info{'fh_fielddata'}, my $fielddata_data,
      ( -s $write_info{'fh_fielddata'} );
...
sub Header {
    ...
    my %hh;
    (
        $hh{'Signature'},          $hh{'Version'},
        $hh{'StructOffset'},       $hh{'StructCount'},
        $hh{'FieldOffset'},        $hh{'FieldCount'},
        $hh{'LabelOffset'},        $hh{'LabelCount'},
        $hh{'FieldDataOffset'},    $hh{'FieldDataSize'},
        $hh{'FieldIndicesOffset'}, $hh{'FieldIndicesSize'},
        $hh{'ListIndicesOffset'},  $hh{'ListIndicesSize'}
...
sub ReadStruct {
    ...
    my ($struct_packed);
    sysseek $fh,
      $hhr->{'gffOffset'} + $hhr->{'StructOffset'} + ( $struct_index * 12 ), 0;
    sysread $fh, $struct_packed, 12;
    my ( $struct_type, $struct_dataoffset, $struct_fieldcount ) =
      unpack( 'V3', $struct_packed );

    $struct->{'ID'}          = $struct_type;
    $struct->{'StructIndex'} = $struct_index;

    if ( $gff->{highest_struct} < $struct_index ) {
        $gff->{highest_struct} = $struct_index;
    }

    $struct->{'Count'} = $struct_fieldcount;
    if ( $struct_fieldcount > 1 ) {
        $struct->{'Fields'} =
          ReadFields( $fh, $hhr, $struct, $struct_dataoffset,
            $struct_fieldcount );
        foreach ( $struct->{'Fields'} ) {
...
sub ReadStruct2 {
    ...
    if ( $struct_fieldcount > 1 ) {
        $struct->{'Fields'} =
          ReadFields2( $fh, $hhr, $struct, $struct_dataoffset,
            $struct_fieldcount );
    }
    elsif ( $struct_fieldcount == 1 ) {
...
sub ReadField {
    ...
    sysseek $fh,
      $hhr->{'gffOffset'} + $hhr->{'FieldOffset'} + ( 12 * $field_index ), 0;
    ...
    {    # BYTE, CHAR, WORD, DWORD
        ...
        my ( $data_length_packed, $exo_string_packed, $stringcount,
            $substrings );
        sysread $fh, $data_length_packed, 4;
        sysread $fh, $exo_string_packed,  unpack( 'V', $data_length_packed );
        ( $objCExoLocString->{'StringRef'}, $stringcount, $substrings ) =
          unpack( 'lVa*', $exo_string_packed );
        my @substr_arr = ();
        for ( my $i = 0 ; $i < $stringcount ; $i++ ) {
            my $str_len;
            my $objSubStr = Bioware::GFF::CExoLocSubString->new();
            ( $objSubStr->{'StringID'}, $str_len ) =
              unpack( 'V2', $substrings );
    ...
    # $objField->{'Value'}=$binary_temp; }
        ...
        sysseek $fh,
          $hhr->{'gffOffset'} +
          $hhr->{'ListIndicesOffset'} +
          unpack( 'V', $field_data ), 0;
        sysread $fh, $number_of_struct_indices_packed, 4;
        my $number_of_struct_indices =
          unpack( 'V', $number_of_struct_indices_packed );
        sysread $fh, $struct_indices_packed, 4 * $number_of_struct_indices;
        my @struct_indices =
          unpack( 'V' . $number_of_struct_indices, $struct_indices_packed );
        my @struct_arr = ();
...
sub ReadLabel {
    ...
    my $label_packed;
    sysseek $fh,
      $hhr->{'gffOffset'} + $hhr->{'LabelOffset'} + ( $label_index * 16 ), 0;
