...
sub new {
    ...
    my $tmp;
    sysread KEY, $tmp, 16;
    my ( $bifcount, $keycount, $offset_filetable, $offset_keytable ) =
      unpack( 'V4', $tmp );

    my $bifhash = {};
    for ( my $i = 0 ; $i < $keycount ; $i++ ) {
        my (
            $resref,        $restype,  $resid,
            $bif_index,     $bif_size, $bif_name_offset,
            $bif_name_size, $bif_name, $index_in_bif
        );
    ...
    my $self =
      { 'path' => $registered_path, 'BIFs' => $bifhash, 'Array' => \%r };
    bless $self, $class;
    return $self;
}
...
sub extract_resource {
    ...
    sysseek(
        BIF,
        24 +
          ( 16 * ( $self->{BIFs}{$bifname}{Resources}{$resource_name}{Ix} ) ),
        0
    );
    my $tmp;
    sysread BIF, $tmp, 8;
