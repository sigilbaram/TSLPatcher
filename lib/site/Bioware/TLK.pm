...
@EXPORT =
  qw( &new &load_tlk &edit_entry &add_entry &delete_entry &save_tlk &string_from_resref &number_of_strings &number_of_stringsFH &tlk_info &add_new_entry &save_TLK &GetStringInfo);
...
sub add_entry {
    ...
    foreach my $entry ( 0 .. ( $self->{'string_count'} - 1 ) ) {
        $exists = CompareEntries(
            $entry,
            $self->{$entry}{Text},
            $self->{$entry}{Sound},
            $self->{$entry}{Flags},
            $self->{$entry}{Length}, @array2
        );
...
sub CompareEntries {
    ...
    if (    ( $entry1[0] eq $entry2[0] )
        and ( $entry1[1] eq $entry2[1] )
        and ( $entry1[2] eq $entry2[2] )
        and ( $entry1[3] eq $entry2[3] ) )
    {
        return $entry;
    }
    else { return -1; }
